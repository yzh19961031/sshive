...
type Event struct {
	...
	Action  string // 仅 EventCommand 使用：execute / blocked
}
...
func (w *AsyncWriter) SendCommand(cmd, result, action string) {
	select {
	case w.ch <- Event{Type: EventCommand, Content: cmd, Result: result, Action: action}:
	default:
	}
}
